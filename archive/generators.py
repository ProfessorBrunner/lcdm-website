...
# RJB: I tried modifying this file to support multiple page directories
# (so everything would not need to be stord under pages in the content
# folder. This quick fix did not work, so I resorted to requiring
# Save_as tags in each file.
...
class Generator(object):
    ...
    def get_files_path(self, files, path, exclude=[], extensions=None):
    ...
    def get_files(self, paths, exclude=[], extensions=None):
        ...
        if type(paths) is unicode:
            self.get_files_path(files, paths, exclude, extensions)
        elif type(paths) is tuple or list:
            for path in paths:
                self.get_files_path(files, path, exclude, extensions)
        else:
            self.get_files_path(files, paths, exclude, extensions)
